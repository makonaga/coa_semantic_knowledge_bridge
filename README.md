...
**検証は us-west-2(オレゴン)で実施しています。** 本ガイドの手順・所要時間・コストの
実測値はすべてオレゴンでの実構築によるものです。東京(ap-northeast-1)に構築する場合は、
各ガイド冒頭の `COA_REGION` を `ap-northeast-1` に設定したうえで、
[ガイド01 ステップ7](install_guide/guide_01_prerequisites.md) の事前確認(AgentCore・
Claude 推論プロファイル・埋め込みモデルの提供状況)と
[ガイド02 ステップ4-4](install_guide/guide_02_deployment.md) のモデル ID 置換を必ず実施して
ください。東京での構築は本ガイドでは**未検証**です。
